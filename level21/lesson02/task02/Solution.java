...
public class Solution {
    ...
    public static boolean isAllModifiersContainSpecificModifier(int allModifiers, int specificModifier) {
        byte[] allMod = ByteBuffer.allocate(4).putInt(allModifiers).array();
        byte[] specMod = ByteBuffer.allocate(4).putInt(specificModifier).array();
        byte[] result = new byte[4];
        for (int i = 0; i < result.length; i++) {
            result[i] = (byte) (allMod[i] & specMod[i]);
        }
        return Arrays.equals(result, specMod);
    }
